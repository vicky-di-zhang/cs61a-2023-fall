def filter(condition, lst):
    ...
    # if del an element, then index i doesnt increase, so next while loop still is index i
    # (but due to already del one element,so acturally read next element)
        if condition(lst[i]) != True:
            del lst[i]
    # if keep this element, then index i increase 1
        else:
            i += 1
...
def has_path(t, word):
    ...
    if len(word) == 1 or is_leaf(t):
        return word == label(t)
    elif not is_leaf(t) and word[0] == label(t):

...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    after_filiter = list(filter(select,paragraphs))
    # note the pro
    if k < len(after_filiter):
        return after_filiter[k]
    else:
        return ''
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def check(x):
        after_spilt = split(lower(remove_punctuation(x)))
        for j in subject:
            for i in after_spilt:
                if j == i:
                    return True
        return False
    return check
...
def accuracy(typed, source):
    ...
    if len(source) != 0 and len(typed) == 0:
        result = 0
    elif len(source) == 0 and len(typed) != 0:
        result = 0
    elif len(source) == 0 and len(typed) == 0:
        result = 100
    else:
        sp_source = list(split(source))
        sp_typed = list(split(typed))
        correct = 0.0
        all = len(sp_typed)
        min_num = min(len(sp_typed),len(sp_source))
        for i in range(min_num):
            if sp_typed[i] == sp_source[i]:
                correct += 1.0
        result =  (correct/all)*100
    return float(result)

    # typed_words = typed.split()  # 将输入字符串分解成单词列表
    # source_words = source.split()  # 将参考字符串分解成单词列表
    
    # # 计算正确的单词数量
    # correct_words = sum(typed_word == source_word for typed_word, source_word in zip(typed_words, source_words))
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    length = len(typed)
    num_of_five = length / 5
    return num_of_five / (elapsed / 60)
    # END PROBLEM 4
